...

{
  ...
  tracer::tracer(int max_depth, std::shared_ptr<Scene> scene)
      : _max_depth(max_depth), _scene(scene) {}
  ...
  vec3 tracer::_trace_reflected(const Ray &_ray, int counter) const {
    ...
    // reflected light
    Ray reflected_ray =
        util::reflected_ray(_ray, min_dist.value().shape_ptr->normal());
    // util::print_vector(reflected_ray.position);
    // util::print_vector(reflected_ray.direction);

    return color + vec3(min_dist.value().shape_ptr->get_material().specular) *
                       _trace_reflected(reflected_ray, ++counter);
  }
  ...
  std::optional<IntersectResult> tracer::_closest_shape(const Ray &ray) const {
    ...
    for (auto const &shape : _scene->get_shapes()) {
      ...
      if ((curr_result && !min_intersection) ||
          (curr_result &&
           curr_result.value().distance < min_intersection.value().distance)) {
  ...
  vec3 tracer::_compute_color(IntersectResult &intersection_result,
                              const Ray &ray) const {
    ...
    for (auto const &light : _scene->get_lights()) {
      ...
      float denom = 1.0f / (attenuation.x + d * attenuation.y +
                            pow(d, 2) * attenuation.z);
      ...
      if (!closest_shadow) {
        finalcolor += algorithm::lighting(light_direction, shape.get(), light,
                                          normal, halfv);
      } else {
