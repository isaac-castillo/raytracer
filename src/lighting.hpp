...
namespace raytracer::algorithm
{
    ...
    inline vec3 lighting(const vec3 &light_direction, const Shape *shape, const Light &light, const vec3 normal, const vec3 &halfvec)
    {
        ...
        vec3 phong = specular * light_color * std::pow(std::max(nDotH, 0.0f), shininess);
