...
namespace raytracer {
  ...
  Sphere::Sphere(float radius, vec3 center, mat4 transformation)
      : Shape(vec3(), transformation), _radius(radius), _center(center) {
    ...
    for (int i = 0; i < slices; i++) {
      for (int j = 0; j < stacks; j++) {
        // Top left
        sphereVertices.push_back(vec4(radius * -cos(2.0f * pi * i / fstacks) *
                                          sin(pi * (j + 1.0f) / fslices),
                                      radius * -cos(pi * (j + 1.0f) / fslices),
                                      radius * sin(2.0f * pi * i / fstacks) *
                                          sin(pi * (j + 1.0f) / fslices),
                                      1.0f));

        // Top right
        sphereVertices.push_back(
            vec4(radius * -cos(2.0f * pi * (i + 1.0) / fstacks) *
                     sin(pi * (j + 1.0) / fslices),
                 radius * -cos(pi * (j + 1.0) / fslices),
                 radius * sin(2.0f * pi * (i + 1.0) / fstacks) *
                     sin(pi * (j + 1.0) / fslices),
                 1.0f));
        ...
        // Need to repeat 2 of the vertices since we can only draw triangles.
        // Eliminates the confusion of array indices. Top left
        sphereVertices.push_back(vec4(radius * -cos(2.0f * pi * i / fstacks) *
                                          sin(pi * (j + 1.0f) / fslices),
                                      radius * -cos(pi * (j + 1.0f) / fslices),
                                      radius * sin(2.0f * pi * i / fstacks) *
        ...
        sphereVertices.push_back(
            vec4(radius * -cos(2.0f * pi * i / fstacks) * sin(pi * j / fslices),
                 radius * -cos(pi * j / fslices),
  ...
  std::optional<IntersectResult> Sphere::inside(const Ray &_ray) {
    ...
    auto world_coord = util::transform_to_world(
        _transform, ray_position + ray_direction * result.distance);
  ...
  vec3 Sphere::normal(const vec3 &vec) const {
    ...
    vec4 normal =
        glm::mat4(glm::transpose(glm::inverse(_transform))) * fake_normal;
    normal.w = 0;

    return glm::normalize(vec3(normal));
  }

  vec3 Sphere::get_center() { return _center; }
